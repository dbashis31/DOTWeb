function indexTableController($scope, $stateParams, IndexOptimizationStat) {

	var schemaId = $stateParams.id;
	$scope.schemaName = $stateParams.name;

	$scope.tables = function() {
		var res = IndexOptimizationStat.tables(schemaId);
	...
	$scope.parentModel = function() {
		var res = IndexOptimizationStat.parentModel(schemaId, "SCHEMA");
		res.success(function(data, status, headers, config) {
