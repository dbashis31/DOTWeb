function uniqueKeyColumnController($scope, $stateParams, UniqueKeyOptimizationStat,
	...
	$scope.columns = function() {
		var res = UniqueKeyOptimizationStat.columns(tableId);
	...
	$scope.parentModel = function() {
		var res = UniqueKeyOptimizationStat.parentModel(tableId, "TABLE");
		res.success(function(data, status, headers, config) {
	...
	$scope.viewDataWindow = function(id, name) {
		...
		$scope.viewData = function() {
			var res = UniqueKeyOptimizationStat.viewData(id);
			res.success(function(data, status, headers, config) {
				// alert(data);
				var allOfTheData = data;
				var dataSource = {
...

	{
		...
		headerName : "Disctinct Percentage(%)",
		field : "htmlString",
		width : 200,
		sortingOrder : [ 'asc', 'desc' ],
		cellStyle : {
